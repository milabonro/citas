...
function Formulario({crearCita}){
  ...
  const [cita,setcitas] = useState(stateInicial);
 
 //actualiza el state
  const actualizarState = e =>{
    setcitas({
      //...cita se mantienen el resto de los elementos porque tomo uno copia del state actual
      ...cita,
...
function App(){
  //cargar las citas de localstorage com state inicial
  let citasIniciales = JSON.parse(localStorage.getItem('citas'));
  if (!citasIniciales){
    citasIniciales = [];
  }

  //useState retorna 2 funciiones
